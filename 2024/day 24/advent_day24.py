...
sys.setrecursionlimit(10**6)  # קבע מספר גבוה יותר לפי הצורך
...
def get_input(path):
    ...
    with open(path) as file:
        ...
        for gate in gates.splitlines():
            ...
            bool_equations.append((v1, op, v2, result))
...
def swap_gates(bit, eq_gates, gates, graph, exp):
...
bad_bits = part_two(msb, variables, graph, bool_equations)

eq_gates = {}

for v1, op, v2, result in bool_equations:
    if result in eq_gates:
        print("ERROR")
...
swaps.append(("z09", "gbf"))
swaps.append(("jgt", "mht"))
swaps.append(("nbf", "z30"))
answer = []

for swap in swaps:
    answer.append(swap[0])
    answer.append(swap[1])

print(",".join(sorted(answer)))
for v1, v2 in swaps:
    eq_gates[v1], eq_gates[v2] = eq_gates[v2], eq_gates[v1]

bool_equations = [(v1, op, v2, key) for key, (v1, op, v2) in eq_gates.items()]

print("strict digraph {")
for gate in bool_equations:
    n1, op, n2, n3 = gate
...
# tp = topological_sort(get_sub_graph(nodes1, graph))

# for gate in tp:
#     if gate[0] not in "xy":
#         print(eq_gates[gate], gate)
